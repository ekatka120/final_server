...
void				RequestHandler::prepareResponse(){
	...
	// else if (_method == PUT && _currentLocation->methods[PUT] ) {
	// 	if (stat(_filePath.c_str(), &buff) == -1){
	// 		responseError(ERR404);//нет такого пути или файла
	// 	} else if (S_ISDIR(buff.st_mode)){
	// 		if (!_currentLocation->index.empty()){
...
int 				RequestHandler::setUpPathFromUrl(size_t lastSlashUrlPos){
	...
	for (std::vector<t_location*>::iterator it = _server->getLocations().begin(); it != _server->getLocations().end(); ++it) {
		...
		if (((*it)->url == _url.substr(0, newLastSlashUrlPos + 1)) || ((*it)->url == _url.substr(0, newLastSlashUrlPos))){
			...
			std::cout << "_url " << _url << std::endl;
			std::cout << "_currentLocation " << _currentLocation << std::endl;
			return (1);
		}
	}
...
//        this->_map_envp["CONTENT_LENGTH"]       = _headers["Content-Length"];
//        this->_map_envp["CONTENT_TYPE"]         = _headers["Content-Type"];
...
//        this->_map_envp["REMOTE_ADDR"]          = _server->getHost();
//        this->_map_envp["REMOTE_IDENT"]         = _headers["__"];
//        this->_map_envp["REMOTE_USER"]          = _headers["_____"];
...
//        this->_map_envp["SERVER_NAME"]          = _server->getServerName();
//        this->_map_envp["SERVER_PORT"]          = _server->getListenSocketFd(); //Боря, это port???
